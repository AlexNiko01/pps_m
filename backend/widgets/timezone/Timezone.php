<?php
namespace backend\widgets\timezone;
...
class Timezone extends Widget
{
    ...
    public function run()
    {
        ...
        if ($user->timezone) {
            $defaultValue = @array_flip(array_keys(static::$timezoneList))[$user->timezone];
        } else {
            $defaultValue = array_flip(array_values(static::$timezoneList))[date_default_timezone_get()];
        }

        echo ChosenSelect::widget([
                'name'=>'timezone_changer',
                'id'=>'timezone-changer',
                'items'=>array_keys(static::$timezoneList),
                'value'=> $defaultValue,
                'options'=> [
                    'encode'=>false,
                ],
        ...
        $url = Url::to(['/site/set-timezone']);

        $this->view->registerJs(<<<JS
$('#timezone-changer').on('change', function(){
	$.get('$url', {offset: this.value}, function(){
	    location.reload();
	});
});
JS
