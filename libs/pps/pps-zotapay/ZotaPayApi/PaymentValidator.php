...
class PaymentValidator
{
    ...
    public static function amountValidate($value, $label, $unsigned = true)
    {
        if (!is_numeric($value) || ($unsigned && $value < 0.003)) {
            throw new ZotaPayValidationException("'{$label}' has not a valid amount value");
        }
    }
    ...
    {
        if (!is_string($value)) {
            throw new ZotaPayValidationException("'{$label}' has not a valid string value");
        }
    ...
    public static function validateRange($value, $label, $min_length = 1, $max_length = 256)
    {
        if (strlen($value) < $min_length) {
            throw new ZotaPayValidationException("'{$label}' has not a valid length. Minimum {$min_length}");
        }
        if (strlen($value) > $max_length) {
            throw new ZotaPayValidationException("'{$label}' has not a valid length. Maximum {$max_length}");
        }
    }
    ...
    public static function digitValidate($value, $label)
    {
        ...
        if (!ctype_digit($value)) {
            throw new ZotaPayValidationException("'{$label}' has not a valid digit value");
        }
    }
    ...
    public static function emailValidate($value, $label)
    {
        if (filter_var($value, FILTER_VALIDATE_EMAIL) === false) {
            throw new ZotaPayValidationException("'{$label}' has not a fully qualified email");
        }
        return true;
    }
    ...
    public static function integerValidate($value, $label, $unsigned = true)
    {
        if ((!is_integer($value) && !ctype_digit($value)) || ($unsigned && intval($value) <= 0)) {
            throw new ZotaPayValidationException("'{$label}' has not a valid integer value");
        }
        return true;
    }
    ...
    public static function validateIntegerRange($value, $label, $min_value = 1, $max_value = 10000)
    {
        if (intval($value) < $min_value) {
            throw new ZotaPayValidationException("'{$label}' has not a valid value. Minimum {$min_value}");
        }
        if (intval($value) > $max_value) {
            throw new ZotaPayValidationException("'{$label}' has not a valid value. Maximum {$max_value}");
        }
    }
    ...
    public static function urlValidate($value, $label)
    {
        if (filter_var($value, FILTER_VALIDATE_URL) === false
            ...
            throw new ZotaPayValidationException("'{$label}' has not a fully qualified URL");
        }
        return true;
    }
    ...
    public static function xmlValidate($value, $label)
    {
        ...
        if (simplexml_load_string($value) === false) {
            throw new ZotaPayValidationException("'{$label}' has not a valid xml value");
        }
        return true;
    }
    ...
    public static function secretValidate($value, $label)
    {
        if (!is_string($value) || strlen($value) < self::SECRET_KEY_MIN_LENGTH) {
            throw new ZotaPayValidationException("'{$label}' has not a valid value");
        }
        return true;
    }
