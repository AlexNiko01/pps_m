 ...
 * @property  $phone
 *
 * HTML code of 3D authorization form, encoded in application/x-www-form-urlencoded MIME format.
 * Merchant must decode this parameter before showing the form to the Customer. The Zotapay System returns
 * the following response parameters when it gets 3D authorization form from the Issuer Bank. It contains auth
 * form HTML code which must be passed through without any changes to the client’s browser. This parameter exists and
 * has value only when the redirection HTML is already available. For non-3D this never happens. For 3D HTML has value
 * after some short time after the processing has been started. .
 * @property  $html
 *
 ...
 * amount, even if status is requested by-request-sn. .
 ...
 */
class CardOrderStatusResponse extends BaseResponse
{
    public $type;
    public $status;
    public $amount;
