...
use pps\cryptoflex\CryptoFlexApi\Requests\crypto\CryptoPayoutRequest;

class CryptoWithdrawRequestTest extends Unit
{
    /**
    ...
    */
    ...
    public function testSuccessCreateOrderRequest()
    {
        ...
        */
        ...
        $this->assertInstanceOf(CryptoPayoutRequest::class, $payout);
        $this->assertStringStartsWith('cryptoflex_', $payout->partner_withdraw_id);
        $this->assertAttributeEquals($this->config->getWalletAddress(), 'wallet_address', $payout);
        $this->assertAttributeEquals('11111111111111111111', 'withdraw_address', $payout);
        $this->assertAttributeEquals('BTC', 'crypto_currency', $payout);
        $this->assertAttributeEquals($this->config->getConfirmationsWithdraw(), 'confirmations_withdraw', $payout);
        $this->assertAttributeEquals(0.0045, 'amount', $payout);
        $this->assertAttributeEquals($this->config->getFeeLevel(), 'fee_level', $payout);

        $this->assertAttributeNotEmpty('sign', $payout);
        $this->assertAttributeNotEmpty('timestamp', $payout);

        $this->assertEquals(CryptoFlexPaymentMethod::CRYPTO, $payout->getMethod());
